...
> 从 `notes/004-A`、`004-B`、`004-C` 提炼,配套 [`01-boot/004` · 加载第一个内核](../book/01-boot/004/)。`01-boot/004` 的 A/B/C 三个 tag 不是三个功能,而是"让第一个 C++ 内核跑起来"时连续踩中的三个坑。它们在源码注释里都留下了修复痕迹,适合做成可查的案例。
