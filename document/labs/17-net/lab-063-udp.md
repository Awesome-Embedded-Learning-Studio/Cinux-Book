---
title: Lab 063 · UDP 协议层验证
---

# Lab 063 · UDP 协议层验证

> 对应 `document/book/17-net/063-udp.md`。验证档 **A 档**:这一章交付的是 UDP 协议层 + IPv4 L4 分派表的还债。可这一章还没 socket API(留下一章),所以「用户态 socket() 发 UDP」做不到——验证靠内核侧的 loopback round-trip + e1000 TX smoke + host 单测,辅以「ICMP 走新表 ping 仍通」这个还债的双保险。
