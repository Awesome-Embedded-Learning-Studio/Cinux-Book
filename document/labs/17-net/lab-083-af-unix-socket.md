---
title: Lab 083 · AF_UNIX loopback echo:把 send/accept/recv 串成 round-trip
---

# Lab 083 · AF_UNIX loopback echo:把 send/accept/recv 串成 round-trip

> 083 章把 `AF_UNIX` 的源码过了一遍:`bind_path`/`connect_path` 加法虚函数、`UnixSocket` 两角色一类、`UnixRegistry` 内存命名空间、立即建连无握手。这个 lab 不发答案,只给路径——咱们按「读 → 改 → 跑」三步,亲手让 loopback echo round-trip 在 Book 工作树上端到端跑通,顺手验证几条 083 章声明的边界(send-before-accept、close 撤名、负路径错误码)。所有断言挂在 `kernel/test/test_socket.cpp` 现有测试上,不需要新写测试文件。

## 你要确认的事

开始之前,先在 Book 工作树(`/home/charliechen/Cinux`)上核这几样源码都在、行号对得上 083 章:
...
如果上面四样有对不上的(比如 `unix_registry.hpp` 不存在是正常的、`sys_socket.cpp` 不在 `sys/` 下),先回头核路径——083 章的链接全是 `kernel/syscall/sys_socket.cpp`,别被旧 dev note 误导。

## 第一步:读懂 loopback echo 的时序

`test_unix_socket_loopback_echo`(`test_socket.cpp:294-344`)是 083 章的 punchline。先读一遍,搞清四方向的时序:
...
- `client.send(msg, ...)` 在 `:308`,**在 accept 之前**。这是 083 章声明的「立即建连,send 可先于 accept」卖点。字节缓冲在 child 的 4 KB RX 环里,等 accept 把 child 取走。
...
083 章声明「立即建连,send 可先于 accept」——别光信书,自己改一次测试看会不会破。**临时**改 `test_unix_socket_loopback_echo`,把 send 挪到 accept **之后**:
...
预期:测试**仍然过**——send-before-accept 只是 083 章强调的卖点(立即建连让 send 能先发),不是 round-trip 的必要条件。把 send 挪到 accept 之后,child 已经被取出来了,send 拷进 child 的环、recv 立刻取走,round-trip 一样通。这反过来证明 `AF_UNIX` 的连接模型比 TCP 宽松——TCP 得先 accept 才能 send,`AF_UNIX` 两个顺序都行。
...
如果你忘了还原,后面跑全量测试会发现这个文件脏——083 章的源码真值是 send-before-accept,别把临时的验证改动留下来。

## 第三步:验证 close 撤名这条链路

083 章有一节专门澄清「早期 dev note 说 close 无 release 钩子 → 不撤名,这条对当前 Book 源码已过期」。release 钩子补上了(`socket.cpp:148-156`)→ `UnixSocket::close()`(`unix_socket.cpp:295-328`)真做 `UnixRegistry::unregister(path_)`(`:325`)。咱们验证一次「close 后重 bind 同名能成功」。
...
083 章有一节讲 ring0 测试内核喂不了用户地址给 `sys_bind`,所以 echo 测试走 `UnixSocket` 直接方法。亲手核一遍这个 陷阱 的根源:
...
跑完上面五步,回头逐条核 083 章的「咱们要点亮什么」七条,每条都能在源码或测试里找到证据:
...
七条全对上,083 章的「教程即验证」才算闭环。如果某条对不上(比如 grep 不到 `prepare_to_wait`、或者 `bind_path` 的行号偏了),回头读 083 章对应小节,看是源码演进了还是章节写错了——教程是 tag-bound 的,以当前工作树的源码真值为准。
...
- 读 `poll_events`(`unix_socket.cpp:391-429`),对比 listening 和 connected 两种角色报的就绪掩码差别(listening 看 accept 队列、connected 看 RX 环 + `POLLHUP`)。这跟 069 `TcpSocket` 的 poll 是镜像。

这些折腾不要求做完,挑一个顺眼的深挖。083 章主线是 bind/connect/echo round-trip,这几个延伸是「顺手吃下的扩展 ABI」的入口。