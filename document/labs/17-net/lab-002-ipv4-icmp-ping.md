---
title: Lab 002 · IPv4 / ICMP / ping 验证
---

# Lab 002 · IPv4 / ICMP / ping 验证

> 对应 `document/book/17-net/002/`。验证档 **A 档**:本章的 punchline 是 `ping 10.0.2.2` 真往返 QEMU SLIRP 网关(发 ICMP echo、收回 echo reply)。验证分两层——先看内核测试把栈在 loopback(确定性)和 e1000(真硬件)上都验过,再看解耦门禁(`check_net_decoupling`)把「栈不依赖驱动」这条规矩机器执行住。
