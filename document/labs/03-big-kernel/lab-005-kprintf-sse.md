---
title: Lab 005 · 给 kprintf 补全格式 + 让 Release 内核起得来
---

# Lab 005 · 给 kprintf 补全格式 + 让 Release 内核起得来

> 配套章节：[005 · kprintf 重构与引导期 SSE 初始化](../../book/03-big-kernel/005/)。这一关不贴答案，给目标、约束和一个等着你修的 `-O2` 崩溃。
...
你得先过 Lab 004：内核能在 `sti` 之后稳定 tick、`int $3` 仍能正常返回。这一关会重新动 kprintf（内核的眼睛）和 boot 入口（内核最早的指令），没把 004 的中断链走稳，这里排错会混乱。

另外提醒一句：这一关的 tag 叫 `005_driver_serial`，但实际没有串口驱动的活——serial 早就有了。别被名字带偏去找串口的事，真正的活是 kprintf 和 SSE。
