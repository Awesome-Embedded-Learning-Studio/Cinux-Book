---
title: Lab 004 · 给每个世界一套页表:地址空间
---

# Lab 004 · 给每个世界一套页表:地址空间

> 配套章节:[004 · 给每个世界一套页表:地址空间](../../book/05-memory/004/)。这一关给你目标和约束,不贴 PML4 拷贝、不贴子树回收、不贴隔离的判断——那些得你自己写。
...
你得先过 Lab 002 + Lab 003。关键依赖:

- `05-memory/002` 的 `VMM::map(virt, phys, flags, uint64_t* pml4 = nullptr)`:这一关的 map/unmap/translate 全部透传给它,把本空间的 PML4 通过那个 `pml4` 参数传进去——`05-memory/002` 说「这个参数是为以后留的」,这一关就是那个「以后」。
- `05-memory/002` 的 `phys_to_virt`(物理地址 + 高半区偏移):读那些存在物理内存里的页表,全靠它。
- `05-memory/001` 的 `g_pmm.alloc_page` / `free_page`:PML4、各级中间页表,都要从 PMM 要页、回收时还页。
...
PML4 半区分界(256)、各级页表项数(512)、`KERNEL_VMA` 这些常量你自己定或沿用 `05-memory/002` 的,这关不提供。
...
`init_kernel` 会打印 `[AS] Kernel PML4 saved at phys ...`,test section `AddressSpace Tests (004)` 全过、末尾 `ALL TESTS PASSED`,就说明地址空间走通了。尤其跨空间隔离那条,是这一关的里程碑。
...
- **析构把用户半区数据页也回收了,后续别处引用的页被抽走**:这套析构设计把用户半区每页当独占,PT 层数据页也 free。别往用户半区映射「还要被别处引用的共享页」。`05-memory/004` 没人这么用不发作,但做共享内存 / COW 时这里是雷区。
...
做到这五条,内核就有了「地址空间」抽象,PMM + VMM + 堆 + 地址空间四块就位。但生产路径里它只做了 `init_kernel`、还没人造实例来住——谁来住?进程。那是下一关(06-process 卷的 `lab-001`)的事。
