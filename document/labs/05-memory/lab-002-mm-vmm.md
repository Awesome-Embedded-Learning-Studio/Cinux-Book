---
title: Lab 002 · 把物理页挂进虚拟地址:VMM map/unmap + demand paging
---

# Lab 002 · 把物理页挂进虚拟地址:VMM map/unmap + demand paging

> 配套章节:[002 · 把物理页挂进虚拟地址:虚拟内存管理器](../../book/05-memory/002/)。这一关给你目标和约束,不贴 walk 的实现,也不贴 demand paging 的判断。
...
你得先过 Lab 001:PMM 的 `alloc_page` / `free_page` 可用——VMM 建中间页表、demand paging 补页,都靠它提供物理页。这一关所有「新建页表」的需求都转成对 PMM 的 `alloc_page` 调用。
