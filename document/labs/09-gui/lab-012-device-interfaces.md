---
title: Lab 012 · 用户态拿到事件、画到屏幕
---

# Lab 012 · 用户态拿到事件、画到屏幕

> 对应 `document/book/08-filesystem/012-userspace-device-interfaces.md`。验证档 **A 档**:这一章交付的是「用户态能 `read` `/dev/event0` 拿事件、能 `mmap` `/dev/fb0` 画像素」。核心机制靠源码阅读 + host 纯逻辑测罩(队列算术、字段填充、边界检查),端到端链路靠 kernel 内测 smoke(`/fb_mmap_test` + `/input_event_test`,test kernel 重挂了 `/dev` 并初始化了测试 framebuffer)+ `make run` 起真内核冒烟(真 VBE + 真鼠标键盘)。
