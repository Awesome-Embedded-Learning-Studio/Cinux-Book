---
title: Lab 004 · 管道:让终端跑起真正的 shell
---

# Lab 004 · 管道:让终端跑起真正的 shell

> 这个实验对应主书 [004 · 管道:让终端跑起真正的 shell](../../book/09-gui/004/),接在 [Lab 003](lab-003-gui-native-app.md) 之后。我们不在 lab 里贴完整答案代码——你要自己从零搭一根内核管道,把它伪装成文件挂到 shell 的 fd 0/1 上,让 003 那个本地回显终端第一次跑起真正的 Cinux shell。
...
- 这个顺序**必须翻转**(002 是"先 fd==0/1 再 VFS"),原因见常见故障第一条——不翻,管道永远走不到。
...
预期:开机进 GUI,终端窗口里出现 Cinux shell 的真实提示符(不是 003 的纯本地回显);敲 `help` 回车,shell 的命令列表回显在窗口里;敲不认识的命令,错误信息也回到窗口。看到 shell 输出真的出现在 GUI 窗口里,整条回路就闭环了。
