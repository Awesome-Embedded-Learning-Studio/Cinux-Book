---
title: Lab 003 · 原生终端应用:让窗口能打字
---

# Lab 003 · 原生终端应用:让窗口能打字

> 这个实验对应主书 [003 · 原生终端应用](../../book/09-gui/003/)。我们不在 lab 里贴完整答案代码——你要自己把"一个能打字的窗口"一层层搭起来。这里只给目标、约束、验证手段和排错方向。

## 实验目标

在 002 的"空窗口"基础上,做出内核里**第一个跑在窗口里的应用**:一个 80×25 的终端窗口。开机进 GUI,点中它拿到焦点,敲键盘,字符带着光标出现在窗口里,写满一行自动换行、写满一屏向上滚动。这一章只做**本地回显**(你敲什么画什么);让它真正跑 shell,是 [Lab 004](lab-004-gui-pipe.md) 的事。
...
- **`handle_key` 实化**:002 它是空桩(`(void)ev;`),现在写成 `if (focused_ != nullptr) focused_->on_key(ev.key);`。注意它**不做命中测试**——键盘只给 `focused_`,鼠标才 `hit_test`。`ev.key` 是 `Event` 联合里的 `KeyEvent`,按引用透传。
...
实现 `write(const char* str, uint64_t len)` 和它调用的几个 helper。`write` 逐字节分发:`'\n'`→`newline()`、`'\r'`→`cursor_x_=0`、`'\b'`→`backspace()`、`'\t'`→`tab()`、其余→`process_char()`;ESC(`\006`)交给任务 5 的 `handle_ansi`。
...
实现 `is_escape(char)`(就是 `ch == '\006'`)和 `handle_ansi(const char* str, uint64_t len, uint64_t& pos)`。
...
- **`set_pipe_write_fd` / `pipe_write_fd` 看着像按键通道,但接了没反应**:这俩在本 tag 是 **dead code**(全仓无生产调用),历史遗留的"裸 fd"方案残骸。真实按键通道是 `stdin_pipe_->try_write`,那是 Lab 004 的事——本 lab 别去碰 `set_pipe_write_fd`。
...
- [ ] 在代码或报告里**诚实标注**两条边界:`static_cast<Terminal*>(focused)` 是硬编码假设(无 RTTI);`set_pipe_write_fd`/`pipe_write_fd` 是 dead code,真实按键通道走 `stdin_pipe_->try_write`(Lab 004 点亮)。不把未接线的东西写成已工作。
