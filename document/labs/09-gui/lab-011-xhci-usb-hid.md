---
title: Lab 011 · xHCI USB HID 驱动验证
---

# Lab 011 · xHCI USB HID 驱动验证

> 对应 `document/book/09-gui/011/`。验证档 **A 档**(用户可见新能力:USB 键鼠输入),但本章是驱动而非用户程序,验证主要靠内核测试 `test_xhci`(找控制器、复位、给设备寻址、跑通 HID 鼠标)+ 一组 host 单测 + grep。本章给内核接上 xHCI USB 主控制器:PCI 发现 → MSI-X 中断 → TRB/环/cycle-bit → 设备枚举 → HID boot 报告 → 收进 008 的 GUI 事件队列。
