---
title: Lab 005 · 位图与图标:让画布画出会镂空的小图
---

# Lab 005 · 位图与图标:让画布画出会镂空的小图

> 对应主书 [005 · 位图与图标](../../book/09-gui/005/)。001 给了我们一块能画像素/矩形/直线/文字的双缓冲画布,但桌面图标是「不规则形状 + 透明背景的小彩色位图」,`draw_rect` 和 `draw_text` 都画不出来。005 补两件东西:一个能逐像素 blit、能跳过透明色的 `draw_bitmap`;以及把「字符画」编译成像素数组的图标数据 + 一个能被鼠标命中的 `DesktopIcon`。**注意边界**:本 lab 你不会把图标摆到桌面、也不会点图标开窗——那是 006。005 只是「颜料和模具」齐了,屏幕上还看不到图标。这里给签名、给约束、给验证手段,不贴完整答案代码。
...
- 已完成 001:理解 `Canvas` 的 back buffer、`pitch_/4` 的「一行多少个 uint32」、`0x00RRGGBB` 像素打包、`flip()` 逐行拷贝。
- 读懂主书第 005 章「为什么 001 不够」「那个没有纯黑的调色板」「DesktopIcon」三节。
...
- 目的地是 back buffer,写入索引沿用 001 公式 `back_buf_[row * (pitch_/4) + col]`——注意 `pixels_per_row = pitch_/4`,不是 `width_*4`,也不是 `pitch_`(那是字节)。把「每行字节数」当成「每行像素数」就错了。
...
实现位置见 [canvas.hpp](https://github.com/Awesome-Embedded-Learning-Studio/Cinux-Book/blob/main/kernel/drivers/canvas.hpp) 声明、[canvas.cpp](https://github.com/Awesome-Embedded-Learning-Studio/Cinux-Book/blob/main/kernel/drivers/canvas.cpp) 定义。001 的 `draw_pixel`/`blit` 是它的参照模板。
...
> 接口约束:`contains` 是 inline 成员,只读不改;`IconAction` 在 005 **只是定义**,「点了图标派发什么动作」的逻辑在 006,本 lab 不要写派发代码。
...
- **QEMU 内核测试**:目标 `run-kernel-test`。
  ...
  预期串口出现 `=== Bitmap Icon Tests (005_gui_bitmap_icon) ===` section 标记,其后逐个 `RUN_TEST` 通过(`test_bitmap_render_opaque`、`test_bitmap_transparent_skip`、`test_bitmap_clip_right`、`test_desktop_icon_contains_inside`、`test_icon_action_values` 等,共 17 个),最终 `TEST_SUMMARY` 全过。这里测的是手工像素 `build_test_icon`,不是 consteval 常量。
...
- **图标整行错位/画面倾斜**:`pixels_per_row` 算成了 `width_*4` 或 `pitch_`(字节)而不是 `pitch_/4`(像素数)。001 就强调过 pitch 是「每行字节数」,换算成「每行多少个 uint32」要除 4。
