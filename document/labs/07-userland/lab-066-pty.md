---
title: Lab 066 · PTY 验证
---

# Lab 066 · PTY 验证

> 对应 `document/book/07-userland/066-pty.md`。验证档 **A 档**:这一章交付的是 Linux PTY ABI(/dev/ptmx 克隆 + /dev/pts/N + master/slave + TIOCSCTTY)。可「真用户程序跑在 PTY 里」的全闭环要 dup2 + 真 shell,留 CFBox——这一章的验证靠 host 纯逻辑单测 + 内核设备测 + boot 冒烟,辅以改公共头 inode.hpp 的回归网。
...
2. **slave 复用 062 的 TTY 行规范**(不重写),回显经 echo sink 路由回 master 读侧;
...
应看到 `class Pty` 的四个方法:`master_write`(每字节喂 slave `TTY::input_char`)、`master_read`(排 master 读环)、`slave_read`(排 `TTY::read_cooked`)、`slave_write`(push master 读环)。文件头注释会画那张四路径表。注意 slave 持一个复用的 `TTY`(062 那块行规范),PTY 的终端感全来自它——不重写。
...
应看到 `TIOCSCTTY` 的 slave inode ioctl override(设 `Task::controlling_tty`)+ `/dev/tty` 的 dynamic lookup(返当前 task 的 controlling_tty 对应 inode)。这把 PTY 跟 session/前台组/信号(062 那套)接通。
