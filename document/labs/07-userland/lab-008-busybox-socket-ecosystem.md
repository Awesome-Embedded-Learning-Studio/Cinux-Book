---
title: Lab 008 · busybox 用户态生态 + socket API 验证
---

# Lab 008 · busybox 用户态生态 + socket API 验证

> 对应 `document/book/07-userland/008/`。验证档 **A 档**:punchline 是 busybox 14 applet 真跑出真输出(真二进制,不是机制测)+ socket loopback echo。busybox 端到端要 build-busybox.sh 编 busybox + ext2 装盘 + QEMU fork+execve;socket 靠内核 loopback echo 测。
