...
> 这个 lab 配套 [001 · 实模式引导](../../book/01-boot/001/)。目标是亲手写出一个"通电 → 读盘 → 点亮屏幕 → 配好图形模式"的实模式两段式引导。这里只给任务、约束和验证手段,**选择子自己算、DAP 自己拼、段自己理**,不贴现成答案。
...
**第一道闸:能构建。** `01-boot/001` 没有运行时自动化测试,构建本身就是冒烟:
