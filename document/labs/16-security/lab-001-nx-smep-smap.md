---
title: Lab 001 · NX / SMEP / SMAP 机制回读验证
---

# Lab 001 · NX / SMEP / SMAP 机制回读验证

> 对应 `document/book/16-security/001/`。验证档 **A 档偏 B**(机制本身无「用户可见现象」,靠**机制回读**——直接读 EFER/CR4 看那几个位设上没——加上跑真程序确认开完三位不炸)。本章拨开 x86_64 的三个硬件保护位:NX(用户数据页不可执行)、SMEP(内核不可执行用户页)、SMAP(内核不可读写用户页)。
>
