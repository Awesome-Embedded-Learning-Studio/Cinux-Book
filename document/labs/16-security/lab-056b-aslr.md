---
title: Lab 056b · ASLR 三件套随机化验证
---

# Lab 056b · ASLR 三件套随机化验证

> 对应 `document/book/16-security/056b-aslr.md`。验证档 **B 档**(机制,无用户可见现象,靠抽样测试 + grep 验证)。本章给用户态布局加 ASLR:栈顶 / mmap 起点 / brk 起点各随机一个页对齐偏移;代码段(text)没随机化(non-PIE 绝对寻址挡着)。验证核心三件事——三个 offset 函数页对齐 + 落在区间 + 每次不同、三个消费点真用了 offset、代码段确实没动。
