---
title: Lab 037 · RingBuffer / dmesg / DMA 池 验证
---

# Lab 037 · RingBuffer / dmesg / DMA 池 验证

> 对应 `document/book/11-foundation/037-ringbuffer-klog-dma.md`。验证档 **B 档**(基建/重构)+ 一点 A(`dmesg` 用户态读得到)。验证靠构建 + 测试 + grep + 看 dmesg 输出。
...
4. run-kernel-test 从 036 的 662 涨到 694(DMA +20)。
...
```bash
git checkout 037_ringbuffer_klog_dma 2>/dev/null || git checkout 037_*
cmake -B build -S . && cmake --build build -j$(nproc)
cmake --build build --target run-kernel-test
```

**期望**:`0 error`;`run-kernel-test` 全绿,总数应在 694 上下(比 036 多 ~20,DMA 那批)。
