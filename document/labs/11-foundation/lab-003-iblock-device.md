---
title: Lab 003 · IBlockDevice 块设备抽象 验证
---

# Lab 003 · IBlockDevice 块设备抽象 验证

> 对应 `document/book/11-foundation/003/`。验证档 **B 档**(基建/重构)。验证靠构建 + 测试 + 看接口形状 + 看解耦是否到位。
...
```bash
git checkout 003_foundation_iblock_device 2>/dev/null || git checkout 003_*
