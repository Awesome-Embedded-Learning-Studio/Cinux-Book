---
title: Lab 080 · ext2 SMP-safe 验证:block_buf_ 替换痕迹 + 双参重载
---

# Lab 080 · ext2 SMP-safe 验证:block_buf_ 替换痕迹 + 双参重载

> 对应 `document/book/08-filesystem/080-ext2-smp-blockbuf.md`。验证档 **B 档**:race 修复是内部重写、这次没搭 host 上的 TSAN 确定性回归,lab 靠「跑 run-kernel-test 全绿 + grep 验收替换痕迹 + 读双参重载注释」三步,亲手确认「`block_buf_` 在 SMP 路径上已被 per-call `KmBuf` 替换干净,且整体没回归」。不改代码——把「block_buf_ 还该出现在哪儿」的判断内化成能自己说出来的纪律。
