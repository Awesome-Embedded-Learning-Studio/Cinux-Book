---
title: Lab 002 · 内核第一次认识「文件」:嵌入式 initrd ramdisk
---

# Lab 002 · 内核第一次认识「文件」:嵌入式 initrd ramdisk

> 配套章节:[002 · 内核第一次认识「文件」:嵌入式 initrd ramdisk](../../book/08-filesystem/002/)。这一关给你目标和约束,不贴 ustar 头布局、不贴 mount 遍历、不贴 embed 流水线的具体命令组合。
...
构建这块要知道 GNU `objcopy` 能把任意二进制文件转成可链接的 ELF object(`-I binary`),以及链接器段(`section`)怎么控制一块数据的排布与加载。`07-userland/002` 嵌入用户程序二进制用的也是同款手法,有印象会更顺。
