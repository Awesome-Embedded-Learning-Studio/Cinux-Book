---
title: Lab 067 · ProcFS 验证
---

# Lab 067 · ProcFS 验证

> 对应 `document/book/08-filesystem/067-procfs.md`。验证档 **A 档**:这一章交付的是 `/proc` 真挂上、`ls /proc` 见 pid、`cat /proc/<pid>/stat` 读到进程信息。注意 ProcFS **没有 host 单测**——它直读 kernel registry(无注入缝,不像 DevFS 的 CharSink),所以核心逻辑走 kernel harness 测;boot 真 `/proc` 靠 `make run` 冒烟。
