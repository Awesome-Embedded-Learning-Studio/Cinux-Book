---
title: Lab 018 · mount/umount 验证
---

# Lab 018 · mount/umount 验证

> 对应 `document/book/08-filesystem/018/`。验证档 **A 档**:这一章交付的是 `sys_mount -t ext2 /dev/sda` 真能挂一块盘、`umount2` 能干净回收、mount factory 四类分支各走各的路。注意挂载表纯逻辑有 host 单测,**factory 四类分支没有**——依赖内核态类(`TmpFs`/`ProcFs`/`DevFs`/`Ext2` + DevFS + BlockRegistry),host 上没有,工厂逻辑验证全在 kernel harness;真挂一块 ext2 盘 + busybox smoke 靠 `make run` 冒烟。
...
```bash
# (1) 运行时 sys_mount 真挂 tmpfs(017 验过,这里复用)
