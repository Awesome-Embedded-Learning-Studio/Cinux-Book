---
title: Lab 003 · 给文件一个统一接口:VFS 内核层
---

# Lab 003 · 给文件一个统一接口:VFS 内核层

> 配套章节:[003 · 给文件一个统一接口:VFS 内核层](../../book/08-filesystem/003/)。这一关给你目标和约束,不贴 `InodeOps` 函数指针表、不贴 `vfs_resolve` 的边界判定、不贴 ramdisk 实现 `FileSystem` 的代码。这一篇只做内核内部的 VFS 管线,系统调用和 shell 在 Lab 004。
...
过 Lab 002:ramdisk 能解析 ustar、列出文件。这一关让那份 ramdisk「升级」成真正的文件系统后端。
...
做到这四条,内核就有了完整的 VFS 管线。但用户态还碰不到——下一关(Lab 004)接上系统调用和 shell。
