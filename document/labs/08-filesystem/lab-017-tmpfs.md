---
title: Lab 017 · tmpfs 验证
---

# Lab 017 · tmpfs 验证

> 对应 `document/book/08-filesystem/017/`。验证档 **A 档**:这一章交付的是 `/tmp` 真挂上、能在里头 `mkdir`/`touch`/`echo`/`cat`、运行时 `mount`/`umount` 真能挂真能卸。注意 tmpfs **没有 host 单测**——核心逻辑依赖 kernel-only 路径(`inode_ref`、`kernel/lib/string.hpp`),host 链不进来,所以机制测走 kernel harness;boot 真 `/tmp` 靠 `make run` 冒烟 + busybox smoke。
...
head -c 8192 /dev/zero > /tmp/big.bin    # /dev/zero 见 010 DevFS
