---
title: Lab 005 · 磁盘上的真文件系统:ext2 只读驱动
---

# Lab 005 · 磁盘上的真文件系统:ext2 只读驱动

> 配套章节:[005 · 磁盘上的真文件系统:ext2 只读驱动](../../book/08-filesystem/005/)。这一关给你目标和约束,不贴 inode 定位数学、不贴单间接块翻译、不贴目录项 rec_len 步进。前提是 Lab 001(AHCI 块读)和 Lab 003(VFS 接口)已就绪。
...
过 Lab 001:`ahci.read(port, lba, sectors, buf_phys)` 能用——这一关所有块 I/O 都靠它。

过 Lab 003:`FileSystem`(mount/lookup)、`Inode` + `InodeOps`、`vfs_mount_add` 都就绪——这一关写一个实现这套接口的 ext2 类挂上去。
