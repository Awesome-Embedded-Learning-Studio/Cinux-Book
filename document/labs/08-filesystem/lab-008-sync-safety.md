---
title: Lab 008 · 并发安全:亲手感受数据竞争,学会「该用哪种锁」
---
...
在输出里找到 `Sync Concurrent Tests (008)` 这一段(由 `run_sync_concurrent_tests()` 驱动)。确认它逐条通过,重点理解每条在断什么:
...
- **任务 4**:跑 `run-kernel-test`,`Sync Concurrent Tests (008)` 段全绿,能解释「为何协作式」。
