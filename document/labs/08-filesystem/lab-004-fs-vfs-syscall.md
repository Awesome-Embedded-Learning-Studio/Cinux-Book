---
title: Lab 004 · 让用户态能 open/read 文件:VFS 系统调用与 shell
---

# Lab 004 · 让用户态能 open/read 文件:VFS 系统调用与 shell

> 配套章节:[004 · 让用户态能 open/read 文件](../../book/08-filesystem/004/)。这一关给你目标和约束,不贴 sys_open 的解析流程、不贴 libc 的 syscall 汇编包装、不贴 cat/ls 的循环骨架。前提是 Lab 003 的 VFS 内核层已经搭好。
...
FDTable 在 host 上镜像测(Lab 003 已有)。系统调用接 VFS 的行为在 QEMU 里验。机内 vfs 系统调用测试(近 700 行)测 open 找到文件、read 读出内容、close 释放、getdents 列目录,以及错误路径(文件不存在、fd 非法、表满):
