---
title: Lab 042 · demand paging 硬门控 + ext2 缓存 验证
---

# Lab 042 · demand paging 硬门控 + ext2 缓存 验证

> 对应 `document/book/13-memory-advanced/042-demand-paging-ext2-cache.md`。验证档 **B 档**(行为变化 + 性能优化)。验证靠构建 + 测试 + grep + 命中计数 + 实机不崩。
...
4. run-kernel-test 从 041 的 730 涨到 734。
...
```bash
git checkout 042_demand_paging_ext2_cache 2>/dev/null || git checkout 042_*
