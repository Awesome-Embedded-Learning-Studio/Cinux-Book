---
title: Lab 001 · VMA 与 mmap 验证
---

# Lab 001 · VMA 与 mmap 验证

> 对应 `document/book/13-memory-advanced/001/`。验证档 **A 档**(用户态 mmap 可见)+ B(VMA 记账内部)。验证靠构建 + 测试 + grep + mmap round-trip。
...
```bash
git checkout 001_vma_mmap 2>/dev/null || git checkout 001_*
