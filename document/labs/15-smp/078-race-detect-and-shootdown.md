---
title: Lab 078 · race-detect 报警器点亮——把哑火链亲手走通
---

# Lab 078 · race-detect 报警器点亮——把哑火链亲手走通

> 对应 `document/book/15-smp/078-race-detect-and-shootdown.md`。验证档 **C 档**(工具/可观测)。race-detect 是 opt-in 检测器,本章 lab 让你亲手把这条「跨 CPU 交错报警器」点亮、看到它真能抓,而不是写新机制代码。核心是把「哑火链」这条隐形链亲手走一遍——option 开了 ≠ 编译宏传了 ≠ 机制测试真跑了 ≠ 测试真 PASS,每一层断掉日志都还是绿的。
