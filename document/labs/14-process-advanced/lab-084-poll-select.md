...
如果上面四样有对不上的,先回头核路径——084 章的链接全是 `kernel/syscall/poll_core.cpp` 这种,别被旧 dev note 误导。

## 第一步:读懂 do_poll_core 的三段循环

`do_poll_core`(`poll_core.cpp:144-218`)是 084 章的脊柱。先读一遍主循环,搞清三段时序:
...
`test_poll_two_fds_one_ready`(`test_poll.cpp:168-189`)是 084 章「多 fd 同时等、只就绪的计返回值」的最小证据。先读一遍:
...
这里有个 084 章点出的 stale:头注释 `poll_core.hpp:17-20` 还写「有限 timeout 只能 yield 自旋、真 timer-wake 是 DEBT」——但这条测试**就是**有限 timeout 真 park + timer-wake 的端到端证据。亲手核一下实现:
...
084 章声明「poll 和 select 两套 ABI、一个核心」——`sys_poll` 直接搬 pollfd 数组,`sys_select` 把 fd_set 位图翻成 pollfd 再喂同一个 `do_poll_core`。这一步用源码核行为一致。
...
084 章声明「POLLERR/POLLHUP/POLLNVAL 三位无条件透传,即使用户没在 events 里请求」。这一步用源码核:
...
跑完上面六步,回头逐条核 084 章的「咱们要点亮什么」七条,每条都能在源码或测试里找到证据:
...
七条全对上,084 章的「教程即验证」才算闭环。如果某条对不上(比如 grep 不到 `timer_queue_arm`、或者 `register_all` 的行号偏了),回头读 084 章对应小节,看是源码演进了还是章节写错了——教程是 tag-bound 的,以当前工作树的源码真值为准。
...
这些折腾不要求做完,挑一个顺眼的深挖。084 章主线是 `do_poll_core` 多路复用层 + poll/select 共享核心,这几个延伸是「顺手吃下的扩展 ABI」的入口。