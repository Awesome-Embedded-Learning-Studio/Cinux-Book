---
title: Lab 002 · clone / futex / TLS 验证
---

# Lab 002 · clone / futex / TLS 验证

> 对应 `document/book/14-process-advanced/002/`。验证档 **A 档**(clone 起线程可见)+ B(共享/复制内部)。验证靠构建 + 测试 + grep + 双线程交错。
...
```bash
git checkout 002_proc_clone_futex_tls 2>/dev/null || git checkout 002_*
