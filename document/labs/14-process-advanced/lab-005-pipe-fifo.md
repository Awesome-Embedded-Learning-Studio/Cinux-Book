---
title: Lab 005 · Pipe 增强与命名 FIFO 验证
---

# Lab 005 · Pipe 增强与命名 FIFO 验证

> 对应 `document/book/14-process-advanced/005/`。验证档 **A 档**:这一章交付的是 pipe 增强(真阻塞/SIGPIPE/O_NONBLOCK)+ 命名 FIFO。punchline 是 FIFO 端到端走通(mkfifo → open 两端 → 跨 fd 往返)+ pipe 写已关读端触发 SIGPIPE。验证靠 host 单测(nonblock 负测 + FIFO cloning)+ kernel 端到端测 + shell mkfifo/fifotest。
