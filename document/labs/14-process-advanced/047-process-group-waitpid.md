---
title: Lab 047 · 进程组与 waitpid 阻塞 验证
---

# Lab 047 · 进程组与 waitpid 阻塞 验证

> 对应 `document/book/14-process-advanced/047-process-group-waitpid.md`。验证档 **A 档**(killpg/waitpid 阻塞可见)。验证靠构建 + 测试 + grep + waitpid 阻塞唤醒。
...
4. run-kernel-test 从 046 的 809 涨到 827。
...
```bash
git checkout 047_proc_process_group 2>/dev/null || git checkout 047_*
