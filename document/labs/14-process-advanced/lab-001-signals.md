---
title: Lab 001 · POSIX 信号 验证
---

# Lab 001 · POSIX 信号 验证

> 对应 `document/book/14-process-advanced/001/`。验证档 **A 档**(能 sigaction 捕获)+ B(投递内部)。验证靠构建 + 测试 + grep + handler 往返。
...
```bash
git checkout 001_proc_signals 2>/dev/null || git checkout 001_*
