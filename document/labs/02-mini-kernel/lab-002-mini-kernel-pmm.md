---
title: Lab 002 · 物理内存管理
---

# Lab 002 · 物理内存管理

> 这个 lab 配套 [002 · 物理内存管理](../../book/02-mini-kernel/002/)。目标:用一张位图把 E820 变成可分配的 4KB 物理页,实现 `alloc_page`/`free_page`,并在 QEMU 内核测试里验它。**位图怎么布局、E820 怎么 carve、链接器符号怎么取**,都自己来,不给现成答案。
...
- 完成 [Lab 001](lab-001-mini-kernel-entry.md):内核有 kprintf、有 QEMU 内核测试框架、串口能输出。
...
- 全程没碰中断/IDT/PIC——那是 [Lab 003](lab-003-mini-kernel-intr.md);也没碰虚拟内存——VMM 是后面。
