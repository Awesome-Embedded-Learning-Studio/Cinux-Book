---
title: Lab 070 · HPET/RTC 时钟验证
---

# Lab 070 · HPET/RTC 时钟验证

> 对应 `document/book/04-developer/070-hpet-rtc.md`。验证档 **B 档**:这一章交付的是两种时钟(monotonic + 墙钟)+ clock_gettime 接线,内部机制。验证靠 host 纯算术单测(换算/解码)+ kernel 机制测(计数器真在走)+ boot 日志(真实时间)。
