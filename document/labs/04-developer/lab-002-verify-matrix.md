---
title: Lab 002 · 验证基建(SMP 唤醒 + 首故障捕获)验证
---

# Lab 002 · 验证基建(SMP 唤醒 + 首故障捕获)验证

> 对应 `document/book/04-developer/002/`。验证档 **B 档**:本章是验证/调试基建,没有用户可见功能。验证两件事——`-smp 2` 那条腿**真的唤醒了 AP**(不再是空转)、故障 dump 的 gated 钩子和 debugcon 捕获**真在源码里且生产字节不变**。
