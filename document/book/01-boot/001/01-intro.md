...
完成后你会看到:QEMU 窗口里先是几行文本(`Cinux Booting...`、`Stage2 OK`、`Mode info OK, switching...`),然后屏幕"啪"地切进图形模式(黑屏,因为还没人往 framebuffer 里画东西),机器安静地停在那里。这就是 `01-boot/001` 的全部——通电、读盘、配屏、待命。
