...
> `10-multitasking/002` 那一章我们把 fork/exec 彻底通了电——子进程会返回 0、CoW 真的写时复制、syscall 跨切换正常。但那套能力还晾在那儿,没人用。这一章,我们把它接进 GUI:点桌面上的终端图标,内核就 fork 出一个子进程、让它 execve 成 `/bin/sh`、跳进用户态跑起来。关键在于——**每个终端窗口背后是一个各自独立的 shell 进程**,你在 A 终端敲命令,B 终端纹丝不动。这是整条 GUI/多任务弧的高潮,也是 Cinux 从「一个进程跑到底」走到「真正的多进程桌面」的终点。
...
这一幕背后,是 `10-multitasking/002` 通电的 fork/exec 第一次被**真正用起来**:

- **点图标 → 生一个 shell 进程**。`create_shell_terminal()` 调 `fork()`,父进程(GUI 工作线程)继续、子进程去 `execve("/bin/sh")`。
- **每个终端一对私有管道**。不是 `09-gui/006` 那种「所有终端共享一对全局管道」,而是每个终端 new 出自己的 stdin/stdout `Pipe`,这个 shell 的输入输出只流进**它自己的**终端。
...
> 一句话:`09-gui/006` 让「点图标开终端」成立,但所有终端共用一个 shell;`10-multitasking/003` 用 fork/execve 让「每个终端一个独立 shell」成立。中间隔的就是 `10-multitasking/001` 的进程原语和 `10-multitasking/002` 的通电。

## 为什么现在需要它

回看 `09-gui/006` 留下的遗憾:点终端图标能开窗口,但 `create_shell_terminal` 接的是**全局** `g_stdin_pipe`/`g_stdout_pipe`——也就是 `09-gui/004` 给那一个 shell 建的管道。于是你开两个终端窗口,它们 `set_stdin_pipe`/`set_stdout_pipe` 接的是**同一对管道**,背后是**同一个 shell**。在 A 窗口敲命令,B 窗口的输出跟着动;两个窗口都不是独立终端,只是同一个 shell 的两个取景器。

要做成真正的多终端,得让「开一个终端」=「生一个新的 shell 进程出来」。而「生进程」正是 `10-multitasking/001` 的 fork、`10-multitasking/001`+`10-multitasking/002` 把它通上了电。所以 `10-multitasking/003` 我们要做的事很明确:把 `create_shell_terminal` 从「new 一个 Terminal 接全局管道」改造成「fork 一个子进程、给它一对私有管道、让它 execve 成 shell」。
