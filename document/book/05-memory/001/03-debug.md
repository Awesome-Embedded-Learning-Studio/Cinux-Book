...
但你会发现一个明显的缺口:我们只能分配**物理**页,却没法把它们挂到**虚拟**地址空间里。现在内核访问内存,用的还是 bootloader 搭的那套固定页表;想给一个新进程做独立的地址空间、想把某个物理页映射到不同的虚拟地址,都没有机制。`03-big-kernel/006` 那会儿为了点亮 framebuffer,我们临时写了那个硬编码页表地址的 `map_mmio`——那只是个 hack,不是正经的虚拟内存管理。
