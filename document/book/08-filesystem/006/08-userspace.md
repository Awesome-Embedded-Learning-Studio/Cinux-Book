...
第二,**`split_pathname` 这一步**。它找到路径里最后一个 `/`,把前面截成父目录、后面截成叶子名。注意 `08-filesystem/006` 里这个函数是**每个 syscall 文件内联一份**的(`sys_creat`/`sys_mkdir`/`sys_unlink`/`sys_rmdir` 各有一份几乎相同的 `split_pathname`)。重复,但能用。把它抽成公共的 `path.cpp`,是后面的事(别急,下一章再说)。
