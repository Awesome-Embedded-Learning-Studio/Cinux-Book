...
先回顾一个容易被忘掉的事实:调度器从 `06-process/002` 起就是**抢占式**的——RoundRobin,每个任务跑 `DEFAULT_TIME_SLICE = 2` 个 tick 就被换下;PIT 从 `03-big-kernel/004` 起就在以 100 Hz 发 IRQ0。也就是说,内核线程随时会被时钟中断打断、切到另一个线程去跑。

而 PMM、堆、调度运行队列、fd 表这些全局可变状态,在 `08-filesystem/007` 之前是**完全裸奔**的。单线程时代这没事,但抢占式调度一来,就是定时炸弹。举三个真实的雷:
