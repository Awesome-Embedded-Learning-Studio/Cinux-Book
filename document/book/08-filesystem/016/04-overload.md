...
(`libs/ext2/ext2_init.cpp:47`)`write_block` / `zero_and_write_block` 的单参版同理(见 `libs/ext2/ext2_init.cpp:73`)。
...
(`libs/ext2/ext2_init.cpp:165`)

把这些 init 期的单线程用法也强改成 `KmBuf` 不是不行,但徒增改动面和回归风险——它们本来就是安全的。留它、并在头文件注释里**明确标注**「`NOT SMP-safe`; SMP uses the dst/src overload」(三个单参版的文档分别在 `libs/ext2/ext2.hpp:120` / `libs/ext2/ext2.hpp:129` / `libs/ext2/ext2.hpp:141`),比删干净更诚实:把「这块代码只能单线程用」明明白白写进注释,而不是删掉信号、留一个看起来人畜无害实则只能在特定时序下用的接口。
