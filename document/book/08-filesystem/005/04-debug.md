...
`08-filesystem/005` 本身没有 notes(pack 里那两条 GP fault / MMIO 碰撞是 `028b`/`028e` 的,属后续 tag,不进本章)。但 ext2 这套有几个高发坑,值得当调试现场。
...
四是 **全局 `g_ext2_instance` 限制单挂载**。InodeOps 的函数签名里没有 `Ext2*`,回调里要读块只能靠一个全局 `g_ext2_instance` 找回实例。这意味着同时**只能挂一个 ext2**——挂第二个会覆盖这个全局,前一个的 InodeOps 回调就指错了实例。这是 `08-filesystem/003` 那套「InodeOps 是自由函数」设计的代价(`08-filesystem/003` 的吐槽里说了,这套后来会被重构成虚函数、实例随 inode 走,这个全局也就没必要了)。这一章老实接受「单 ext2 挂载」的限制。
