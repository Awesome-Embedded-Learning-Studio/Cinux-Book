...
到这里,内核挂载的是一块**磁盘上、按 ext2 标准布局、能读**的真文件系统了——`08-filesystem/002-003` 的 ramdisk 被它取代,`08-filesystem/004` 的 `cat`/`ls` 不用改一行就能读 ext2 盘。但这个 ext2 是**只读**的:`ext2_file_read` 能读,却没有 `create`/`write`——你不能在 shell 里 `touch` 一个文件或写点什么。

下一站(`08-filesystem/006`),给 ext2 加上**写**:分配新 inode、分配数据块、写目录项、更新位图和统计。有了写,ext2 才算个「能用的」文件系统,而不是只读快照。再往后(`08-filesystem/007` 及之后)还有 `cwd`/`stat`、同步安全、init 线程模型等一串打磨。不过那是后面的事,我们先把「能从磁盘读 ext2 文件」这个里程碑坐实。
...
- `08-filesystem/003` 章 · [给文件一个统一接口:VFS 内核层](../003/):ext2 实现的就是 `08-filesystem/003` 定义的 `FileSystem`(mount/lookup)和 InodeOps,挂进同一套 VFS——本章是 `08-filesystem/003` 抽象的第一个「真」后端。
- `08-filesystem/001` 章 · [让内核自己找到磁盘:PCI 枚举与 AHCI 驱动](../001/):ext2 的所有块 I/O 都走 `ahci.read(port, lba, sectors, buf_phys)`。
