---
title: 068 · ext2 间接块:把 double-indirect 真做了,撤掉那个块大小的 workaround
---

# 068 · ext2 间接块:把 double-indirect 真做了,撤掉那个块大小的 workaround
...
>
> A 档:punchline 是 822 KB 的文件(那个 ldso)真能在 1024 字节块的 ext2 上完整读回来——822 KB 远超 single-indirect 的 268 KB 上限,必定走 double-indirect。这一章真正要讲的是 ext2 的**三级块映射**(direct / single-indirect / double-indirect)怎么用一层套一层的指针索引大文件,以及写路径里那个「单缓冲被覆盖」的陷阱怎么躲。
...
3. **写路径的头号坑:单缓冲被覆盖**:ext2 驱动只有一个 4 KB 的 scratch buffer,每写一块都覆盖它。分配下层块、写盘之后,改上层指针前必须**重新读回上层块**,否则改的是被覆盖后的垃圾。
...
然后三层下钻:`i_block[13]` → 第 idx1 个 single-indirect 块 → 第 idx2 个数据块。读路径(纯读三层,hole——`disk_block == 0`——落公共零填路径)和写路径(lazy-alloc 三层)镜像这套算术。

## 写路径的头号坑:单缓冲被覆盖

写比读麻烦,因为要**分配**还不存在的间接块。这里有个这一章最大的坑,值得细讲。

ext2 驱动只有一个 scratch buffer `block_buf_`(4 KB,一块大小)。每次 `write_block()`(把 buffer 内容写回盘)都用它——也就是说,buffer 里**同一时刻只装得了一块**。分配 double-indirect 数据块的过程要碰三层:double-indirect 块本身、某个 single-indirect 块、最终的数据块。每一层分配完、写盘之后,buffer 就被覆盖了;这时要回头改**上一层**的指针(把新分配的下层块号记进去),改的是哪个 buffer?是刚被覆盖的垃圾。

正解是**二次 read-modify-write**:分配并写下层块后,**重新 `read_block()` 读回上层块**,在(重新读到的)上层 buffer 里改指针,再写回上层。三层逐层下钻、逐层落盘,每一层回头改上一层前都重新读(`ext2_inode.cpp:431`):

```cpp
// Re-read the double-indirect block: writing the child clobbered buf.
read_block(di_blk, block_buf_);
// 在 block_buf_ 里改 idx1 槽的指针,再 write_block(di_blk, ...)
```

> 这套「写完下层、回头改上层前重新读」的 dance,跟现有 single-indirect 的二次 read-modify-write 一个模子。错一步就毁 inode 元数据(指针写错位置),而且不一定立刻崩——可能下次读那个文件才出错,debug 起来特别费劲。所以这一层必须跟 single-indirect 的既有写法严格对齐,不能自创一套。
...
- double-indirect 三层算术:`offset = file_block - (direct + ptrs)`,`idx1 = offset/ptrs`(double 块里哪个 single 指针)、`idx2 = offset%ptrs`(那个 single 块里哪个数据指针)。读路径纯读三层(hole 落零填),写路径 lazy-alloc 三层。
- 写路径头号坑:scratch buffer 单块,写下层后改上层指针前必须**重新读回上层**(二次 read-modify-write,跟 single-indirect 既有写法对齐),否则改的是被覆盖的垃圾、毁 inode 元数据。顺带放开 write 的 file_block 上限(原来只让写 direct)。
