...
先看清 `08-filesystem/008` 的启动路径长什么样。`kernel_main` 里大致是这个顺序:
...
- `08-filesystem/008` 的 `Mutex`/`Semaphore` 想用在启动路径(比如等磁盘)?没处下嘴。
- 那个临时的 `run_concurrent_stress` 框架,本来是 `08-filesystem/008` 用来验证并发安全的脚手架,现在却**兼着**「启动到 shell」的职责——两件事纠缠在一个函数里,不干净。

`08-filesystem/009` 要做的就是解耦:调度器在 `kernel_main` 里直接初始化,然后 spawn 一个 `kernel_init` 线程,让**它**去挂文件系统、起 shell。这样一来,启动逻辑就跟普通内核线程一样,能被调度、能阻塞、能让出——`08-filesystem/008` 的阻塞原语终于有了用武之地。
