...
- **静态节点**:`/proc/version` 和 `/proc/uptime` 这些不依赖进程的节点没做,留 follow-up。`/proc/meminfo` 和 `/proc/cpuinfo` 已经做了——前者从 `g_pmm` 读 MemTotal/MemFree(其余 buffer/cache/swap 诚实地读 0,因为 CinuxOS 没跟踪),后者从 `g_acpi_info` 的 MADT 枚举每 CPU 的 `processor / apicid / model name` 块(busybox `nproc` 数 `processor :` 行)。实现见 `procfs_content.cpp:63`(`format_proc_meminfo`)/`:77`(`format_proc_cpuinfo`),readdir index 2/3 挂上(`procfs.cpp:180/188`),`ProcFs` 类有 `meminfo_inode_`/`cpuinfo_inode_` 成员(`procfs.hpp:51-52`)。
