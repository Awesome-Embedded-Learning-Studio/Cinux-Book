...
grep `kernel/test/` 没有 `test_timer_queue.*`——整棵树只有 `14-process-advanced/007` 的 `test_poll_finite_timeout_parks_then_returns_zero`(`kernel/test/test_poll.cpp:191`):
...
这条链跑通 = timer_queue 端到端验过。所以本章引用 timer_queue 时标「被 `14-process-advanced/007` 间接验证」,不是「有单测」——这是诚实的措辞。
...
还有个容易混的点:`test_memory_stats.cpp`(`kernel/test/test_memory_stats.cpp:21`):
...
- **timer_queue**:没 gate,永远编进 kernel(`timer_queue.cpp` 在 `kernel/proc/CMakeLists` 里无条件链),被 `14-process-advanced/007` poll 吃。
...
2. **没核 timer_queue 的 SMP 安全细节**——表是全局单例 `g_entries[32]`,AP 也调 `Scheduler::tick`(AP 起来后同样走 tick 路径),多核 tick 都拿同一 `g_lock` spinlock,理论安全,但没压力测试。`14-process-advanced/007` 的单次 30ms 超时不触发 `kMaxTimers` 表满、不触发多核并发 arm 等边界。
...
5. **nanosleep 未迁仍 yield-poll**——`timer_queue.hpp` 头注释(`kernel/proc/timer_queue.hpp:5`)提了 nanosleep 是 timer_queue 的目标用户,但 `sys_nanosleep.cpp:50-55` 还在 yield 自旋。**以 `.cpp` 为准**,nanosleep 这条现在其实没生效——`timer_queue` 真正的用户是 `14-process-advanced/007` poll。
...
- band 0 但 sti/hlt → tick 唤醒后 quantum 没耗尽继续选自己,init/child 永远 Ready,gate 卡在第一次 fork、#PF 全程 +0(与 `14-process-advanced/005` 章 sti/hlt 坑同族,机制不同:`14-process-advanced/005` 是陷阱帧损坏 #DF,这里是同级任务饿死)。
