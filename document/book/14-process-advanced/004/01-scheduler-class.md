...
> 两个目标。一是把调度器从"写死的一种轮询"改成**可插拔的调度策略**(调度类),并加上**优先级**——高优先级先跑、同优先级轮询。二是兑现 `14-process-advanced/003` 留的债:`SIGSTOP`/`SIGCONT` 之前只改了信号状态,没有真调度效果(停掉的任务其实还在被调度)。这一章给它加上"停止"状态,让被 STOP 的任务**真不被调度**,CONT 时真恢复。
...
`14-process-advanced/003` 的 SIGSTOP/SIGCONT 只改了信号侧状态。这一章给任务加一个**停止状态**,让它真起调度效果:
