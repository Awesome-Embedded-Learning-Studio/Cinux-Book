...
先说为什么是现在。到 `03-big-kernel/005` 为止,内核已经能在 `sti` 之后稳定响应时钟中断,异常链也走通了。但系统的「脸」还是黑的。接下来无论是想做个 shell、想画个简单的图形界面,还是仅仅想在调试时**不依赖串口窗口**就能看见内核在说什么,都绕不开「先把屏幕点亮」这一步。它是第一个真正「看得见」的驱动,也是后面所有用户态交互的舞台。

再往回看一层,这一章还顺手兑现了 `03-big-kernel/005` 埋下的一个承诺。`03-big-kernel/005` 把 kprintf 的格式化引擎和输出后端用回调解耦时,我们说过「`03-big-kernel/006` 之后还会多一路屏幕,到那时你就明白这层回调当初为什么要抽出来了」。这一章先把屏幕这路硬件备好(像素能画、字能显);真正把 kprintf 接上来、让那句承诺落地,是 `03-big-kernel/007` 的事。所以这两章是一条线的两段:`03-big-kernel/006` 解决「屏幕能不能画」,`03-big-kernel/007` 解决「kprintf 说不说得到屏幕上」。
