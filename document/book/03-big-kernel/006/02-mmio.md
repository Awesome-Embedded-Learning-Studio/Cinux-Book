...
也就是说,VBE 这套机制更早的 boot tag 就搭好了,本章只是把目标模式从 `0x118`(1024×768×24)重定向到 `0x144`(1024×768×32),顺手把 `vesa_set_mode` 顶部那条 `// TODO: Step 1: Call BIOS INT 0x10 ...` 注释改成了描述性注释。注意是「改注释」,不是「填代码」——函数体里那条 `int $0x10` 从 `03-big-kernel/005` 就在那儿,`03-big-kernel/006` 一字没动,真正变的只有模式号常数和这一行注释。所以别误以为「VBE 模式切换是 `03-big-kernel/006` 新加的」——它早就在,`03-big-kernel/006` 只是换了个更高位的模式号、清掉了那条已经过时的 TODO 注释。
...
这两个地址是 bootloader/mini kernel 当初放页表的地方(并且把它们映射到了高半区,所以内核能按这两个虚拟地址访问到表本身)。`map_mmio` 就把这两个地址当指针,直接改表项。这是一个**最小、有效、但脆弱**的方案:它假设页表就在这两个固定位置、假设表的结构是标准的 4 级分页。等以后(`05-memory/001`、`05-memory/002` 那一带)我们做了正经的页表管理器,这种「硬编码地址摸进页表」的写法会被替换掉。但此刻,为了点亮屏幕,它够用,而且干净。
