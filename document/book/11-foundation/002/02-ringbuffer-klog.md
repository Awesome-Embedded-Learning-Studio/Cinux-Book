...
先看环形缓冲。在 `11-foundation/001` 之前,`kernel/` 里有两套手写的环形缓冲,而且**策略还不一样**:
...
第二件事。`11-foundation/001` 之前内核只有 `kprintf`——它逐字符往串口/屏幕吐,是**实时**的:你站在屏幕前能看见,但启动一过、画面一滚,之前发生过什么就没了。想排查一个 late-init 的问题,或者让用户程序知道内核启动经历了什么,都抓瞎。
