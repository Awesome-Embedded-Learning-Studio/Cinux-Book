...
`11-foundation/002` 之前,ext2 和 AHCI 绑得很深,至少三处:

- **构造函数吃 `AHCI&`**:ext2 直接持有一个 AHCI 控制器引用,知道它是 AHCI;
- **自带一摊 DMA**:ext2 自己 `alloc_pages` + 硬编码偏移 + 手动映射,搞一套 DMA 缓冲(和 `11-foundation/002` 收掉的散装 DMA 同一种病,只不过这份长在 ext2 里);
...
**接口走 `ErrorOr<void>`,不走 `bool`。** 和 `11-foundation/001` 一脉相承——这是个会失败的 IO,失败原因(IO 错)该有名字、有类型,而不是一个 `false` 让调用方猜。它是纯内核内部接口(不跨 syscall trap),所以直接用 `ErrorOr`,不用像 syscall 关口那样翻成 `-errno`。
...
**`AHCIBlockDevice`**(`kernel/drivers/ahci/ahci_block_device.hpp`)——把老的 `AHCI::read/write` 包成 `IBlockDevice`。它持一个 DMA 缓冲(从 `11-foundation/002` 的 `DmaPool` 来),`read_blocks` 走 DMA 把数据搬到调用方缓冲。注意它**不碰 `ahci.cpp` 本体**——只是在 AHCI 外面包一层适配器。
