...
runqueue 是有锁的(`irq_guard`),所以**不是 runqueue 竞态**——坏的是 task 的 ctx 字段本身。`15-smp/003` 的多核安全有个假设:「`pick_next` 把任务从队列摘走,就不会有两个核同时跑同一个任务」。这个假设挡住了「两核同时**运行**同一任务」,但**漏了迁移窗口**:旧核还在存、新核已经开始取。摘出队列和「上下文存完」之间有个时间差,窗口就在这儿。
