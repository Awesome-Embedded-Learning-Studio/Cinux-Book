...
> 到 `17-net/004` 章为止,`AF_INET` 这条腿已经站得很稳。这一章给 Linux socket 抽象里的另一条腿 `AF_UNIX`(本地命名空间 socket)铺一条不走网卡的近道——同样的 `socket()`/`bind()`/`listen()`/`accept()`,只是地址从 `(IP, port)` 换成一个文件系统风格的名字,字节不经网卡直接从一个 socket 拷到对端的 RX 环里。
