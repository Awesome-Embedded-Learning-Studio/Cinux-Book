---
title: 059 · musl 静态移植:对齐 Linux ABI、铺初始栈,以及一个被 SMAP 拦下的潜伏 bug
---

# 059 · musl 静态移植:对齐 Linux ABI、铺初始栈,以及一个被 SMAP 拦下的潜伏 bug
...
**只静态链接,没动态链接。** 这一步的 musl 程序是把 `libc.a` 静态链进可执行文件,没有 `ldso`、没有 `PT_INTERP`、没有 `.so`。动态链接(内核加载 `ld-musl`、处理 `PT_INTERP`、运行时重定位)是后面的事。
