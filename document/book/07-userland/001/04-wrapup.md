...
- `05-memory/004` 章 · 地址空间:`AddressSpace` 的「内核半区 `PML4[256..511]` 共享、用户半区私有」设计——`launch_first_user` 为什么要手动把 framebuffer 的 identity-mapping 复制进用户 PDPT,就是对上这个设计。
