...
> **tag-bound 说明**:这一章讲的 `Window`(`x_/y_/w_/h_` + `title_[64]` + `canvas_` + 静态 `TITLE_BAR_HEIGHT=20` / `CLOSE_BUTTON_SIZE=14` + `next_id_`)是 030 当时**内核内**的简版。后续的 visor 解耦把整个 GUI 整体外置到 `third_party/Cinux-GUI/`,在那里 `window.hpp` 被重写成基于 `Widget` 的复合控件(字段变成 `kTitleBarHeight` / `kCloseButtonSize=20` / `theme_` / `content_` / `on_close_` / `drag_px_` 等,和这一章的简版不再是一回事)。本章一律按 tag 030 当时的内核源码叙述,链接也指向该 tag 的 blob。
