...
>
> 这一章真正讲的不是「设备节点本身」——那个 `08-filesystem/010` 已经立过了(`InodeOps` 子类 + `add_node` 投影到 `/dev`)。这一章讲的是把**两类已经存在的设备能力**暴露给 ring3 的两条不同形态:`/dev/event0` 是「事件流」,靠 `read/poll` 把内核 input event 排到用户态;`/dev/fb0` 是「内存窗口」,靠 `mmap` 把 VBE 物理显存绑进用户进程。两个设备都只是 `InodeOps` 基类的两套不同 override 子集——`08-filesystem/010` 立的节点机制一行没动,本章给它补两个真设备 inode 子类。
>
> 边界先说在前头:`/dev/event0` 是 flat 节点(不是 Linux 的 `/dev/input/event0`,DevFS 不为 input 子树投影层级——PTY 走的是 `dynamic_lookup` 投 `/dev/pts/<N>`,但 input 子树没有对应的 resolver,要扩 follow-up);`Event` 结构是 Cinux 自己的 `cinux::gui::Event`,不是 Linux 的 `input_event`,用户态要 mirror 它的字节布局(字段顺序和类型,不是 packed);`poll` 是 level-triggered 不是 edge;事件队列满了静默丢(对齐 GUI EventQueue 策略);NVMe 不放这一章(`12-storage/002` 讲过 NVMe 驱动)。还有一条验证上的诚实:`poll` 路径在 dev note 里明说「只设计、未实跑」——本章的设计讲透了,但「真在 QEMU 里跑过」只能盖章到 `read` 路径,`poll` 等 ring3 GUI host 真用上时再盖。
...
回到这一刻为止系统有什么。`08-filesystem/010` 立了 DevFS 节点机制(`InodeOps` 子类 + `add_node`),把 `/dev/null`/`zero`/`console` 挂到了 `/dev`,但那几个节点是「内核里有个 sink,读写触发 sink 动作」——设备能力全在内核里。`03-big-kernel/006` 把底层 VBE framebuffer 点亮了(bootloader 给的 `PhysBasePtr` + 2MB 大页映射进内核高半区 `KMEM_FB_BASE`),`Framebuffer::put_pixel` 在内核态直接画屏幕。`03-big-kernel/008` 把 PS/2 scancode 解析做完了,`mouse.cpp` 三字节包解成 `MouseEvent`、`keyboard.cpp` scancode 翻成 `KeyEvent`。`09-gui/001-006` 这卷前面几章,GUI 画布、窗口管理器、原生应用、位图图标、桌面,全在内核态用这块 fb + 这套 input 画窗口。
...
(A) 是「事件流」:硬中断进来,内核解码完(`mouse.cpp` 解 PS/2 三字节包、`keyboard.cpp` 解 scancode,`03-big-kernel/008` 讲过的不重讲),`push_event` 把 `Event` 塞进 `RingBuffer`(`Spinlock` 守护,因为有两个生产者——不能复用 GUI 那把 lock-free SPSC 队列),用户态 `read/poll` 排干。
