...
先点一个对照:`/dev/event0` 是「**流式设备**」(靠 `read/poll`,事件一条条流过去),`/dev/fb0` 是「**内存式设备**」(靠 `mmap`,把显存整个窗口暴露给用户态直接读写)。所以 `FramebufferDevOps` **只 override `mmap + ioctl`**——`read/write` 走 `InodeOps` 默认 `NotImplemented`,基类替你拒掉(下一节展开)。看 `kernel/drivers/video/fb_dev.hpp:23-29`:
...
`mmap` 全文见 `kernel/drivers/video/fb_dev.cpp:35-47`:
...
`phys_base()` 是 `Framebuffer` 暴露的 VBE PhysBasePtr,见 `kernel/drivers/video/framebuffer.hpp:98`:`uint64_t phys_base() const { return phys_base_; }`。这个字段是 `Framebuffer::init` 时存的,见 `kernel/drivers/video/framebuffer.cpp:17-23`:
...
bootloader 给的 VBE 物理地址,`init` 存进 `phys_base_`,本章消费这个字段。`03-big-kernel/006` 讲过 `init` 里那个 2MB 大页映射进 `KMEM_FB_BASE`,这里不重讲页表——只点「`phys_base_` 是 `03-big-kernel/006` 留的字段,本章消费」。

### sys_mmap:登记意图(IoPhys VMA)

真正建映射不是 mmap 钩子的活,是 `sys_mmap` + page-fault 的活。看 `kernel/syscall/sys_mmap.cpp:131-143` 的 device probe:
...
`NotImplemented` 当成「这不是设备 mmap」的信号——普通文件/匿名映射走下面那条 page-cache 路径。命中了就把物理地址存下来,给 VMA 打 `IoPhys` 标志,见 `kernel/syscall/sys_mmap.cpp:181-183`:
...
然后在 VMA 落定之后,把物理地址写进 `vma->phys_base`,见 `kernel/syscall/sys_mmap.cpp:191-196`:
...
注意此刻用户态那段虚拟地址**还没 PTE**——demand paging。`IoPhys` 标志的定义见 `kernel/mm/vma.hpp:63-68`:
...
用户态拿到 mmap 返回的虚拟地址,首次写像素时触发 `#PF`——这时候才真正建 PTE。page-fault handler 的 IoPhys 分支见 `kernel/arch/x86_64/page_fault.cpp:256-283`:
...
光有 mmap 不够——用户态 GUI host 还得知道屏幕多大、一行多少字节,才能 size 自己的 canvas。这就是 `ioctl(FBIOGET_SCREENINFO)` 的活,见 `kernel/drivers/video/fb_dev.cpp:49-67`:
...
回填结构是简化的 4 字段,见 `kernel/drivers/video/fb_dev.cpp:27-32`:
...
3. **`kFbioGetScreenInfo = 0x4600`** 是 Cinux 自定义常量,见 `kernel/drivers/video/fb_dev.hpp:34-37`。注释明说「Mirrors Linux FBIOGET_VSCREENINFO in role, not in struct layout」——**数值碰巧和 Linux `FBIOGET_VSCREENINFO` 一致,但 struct 布局完全不同**,用户态别照搬 Linux 用户态结构体,得 mirror 这 4 个字段。
