...
要做"窗口里能放不同内容",所有能画、能命中、能收事件的东西得共享一个虚接口。这就是 `libs/gui/core/widget.hpp:41-136` 的 `Widget`。它的核心是三个虚 hook:
...
**`flatten` 是非虚的框架入口,`paint_to_list` 才是子类填的虚 hook。** 这样 clip push/pop 和递归子控件由框架统一管,子类只管"画我自己",不用操心"我的祖先矩形是啥""我的孩子要不要递归"。看 `libs/gui/core/widget.cpp:33-43` 的实现就一目了然:
...
**`hit_test` 默认按"子控件后画的在上、先命中"递归。** 看 `libs/gui/core/widget.cpp:45-56`:children 从后往前(last-to-first)递归,因为后 add 的 child 画在上面、应该先被点中;都不命中才轮到自己。子类可以 override 成非矩形的命中形状(比如圆角按钮),或者像 Window 那样自定义命中逻辑(标题栏、关闭键、内容区各走各的)。

**`on_pointer`/`on_key` 默认 noop。** 老的、不需要响应输入的控件照样能跑;新控件 override 掉就自动接管了对应事件。注意键盘事件 `on_key` 收的是 `libs/gui/core/event_payload.hpp:41-45`(ascii + scancode + modifiers 三字节),不是 `09-gui/002` 那种 `KeyEvent`——这是因为事件要跨进程传输(`09-gui/012` 的 `/dev/event0` 走的就是这套 wire layout),payload 必须 packed、定长。
...
控件不直接画像素了,那它产出的"我想画什么"住哪儿?就是 `libs/gui/core/paint_list.hpp:109-138` 的 `PaintList`——一张定长的绘制指令数组:
...
有了清单,谁来执行?就是 `libs/gui/core/compositor.hpp:40-78` 的 `Compositor`。它的 `render` 遍历 `PaintList`、逐条 cmd 调对应的处理函数:
