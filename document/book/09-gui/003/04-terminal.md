...
回到这一章的主角——`libs/gui/core/widget/terminal.hpp:32-97` 的 `TerminalWidget`。它继承 `Widget`,override 了三个 protected hook:`paint_to_list`(画)、`collect_dirty`(报告脏区)、`clear_dirty`(清脏)。它的全部"内存"就是几张并行数组加一个光标:
...
**`fg_colors_`/`bg_colors_` 存的是 ANSI 调色板**索引(0..255),不是 XRGB8888 像素值。索引到像素的翻译在 `paint_to_list` 才做——查 `libs/gui/core/widget/terminal.cpp:19-33`:0..15 是 `libs/gui/core/colors.hpp:10` 的标准 16 色(黑红绿黄蓝品青白 + bright),16..231 是 6×6×6 立方(每通道取 `0` 或 `55+40*v`),232..255 是灰阶(`8+(idx-232)*10`)。这覆盖了 xterm-256color 的全套色,`ls --color`、彩色 prompt、vim/less 的高亮都能出来。**全程纯整数,无浮点**——这是 GUI 核心的一条铁律(swraster 用 Q8.8 定点也是同源)。
...
字符缓冲是语义层,真正产出绘制指令靠 `libs/gui/core/widget/terminal.cpp:390-418`:
...
保留模式的核心红利就是脏区重绘。TerminalWidget override 了 `libs/gui/core/widget/terminal.cpp:349-379`,只报告真正变化的矩形:
