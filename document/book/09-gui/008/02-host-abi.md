...
这道边界两边靠什么通话?靠一张纯数据的函数指针表。它在 `libs/gui/core/host.hpp` 里,叫 `Host`:
...
（`libs/gui/core/host.hpp:56` 起。）这张表里全是**函数指针**——core 永远不直接调 framebuffer 或鼠标队列,它调的是 `host->core.flush(...)` 这种间接调用。这就引出这一整弧最关键的一句话:**谁填这张表,core 就跑在谁上面**。
...
（`libs/gui/core/host.hpp:42`。)注意 `pixels`/`stride`/`width`/`height`/`format` 全是 **core 预填好的**——staging 缓冲的归属权在 core 这边,host 只是被邀请进来「在这块缓冲上画」。host 写回的只有 `rects[]` 和 `count`:这一帧它弄脏了哪几块。**core 拥 staging,host 只收脏帧**——这是 flush 显示模型的核心,它让 core 不用关心 host 怎么画、画给谁看。
...
（`libs/gui/core/gui_core.cpp:44`,idle 跳过在 `libs/gui/core/gui_core.cpp:79`。) `pump()` 自己**做不了任何 GUI 的事**——它没有窗口管理器、没有渲染器、不知道屏幕在哪。它只是个协议骨架:排空输入、把 staging 缓冲递给 host 让它画、把 host 报告的脏区收拢、推走。整个函数对宿主是什么类型一无所知,这就是「不感知自己跑在内核态还是用户态」的实现机制。
...
Cinux 给这套几何立了一条硬规矩,写在 `libs/gui/core/region.hpp:10` 顶部:**宁可多推,绝不漏推**。多推几个没变的像素,是看不见的性能损失;漏推一个变了的像素,是屏幕上残留旧内容的视觉 bug。这条规矩贯穿下面所有的设计。

脏矩形运算建立在**半开区间**的 `Rect` 上:``libs/gui/core/region.hpp:47`)。
...
（`libs/gui/core/region.hpp:113` 起,容量常量 `kMaxRects = 32` 在 `libs/gui/core/region.hpp:116`。)定容、`add()` 不分配堆——这是为了让最常见的「一帧累积脏区」这条热路径能在任何上下文里跑。那塞到第 33 个怎么办?它不报错也不扩容,而是把当前所有矩形 + 新矩形的**包围盒**算出来,整个 `Region` 坍缩成这一个矩形:
...
（`libs/gui/core/region.cpp:51`。)这就是「宁可多推绝不漏推」的落地:包围盒盖住了所有原本的矩形(可能也盖了些本来没变的像素),但绝不会漏掉任何一个变了的像素。对脏区来说,过覆盖是性能问题,欠覆盖才是正确性问题,所以一律取前者。
