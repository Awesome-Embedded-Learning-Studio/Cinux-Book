...
工具链卷到这里就齐了:你能编译、能链接、能拼出 BIOS 认的镜像、能塞进 QEMU 跑、能让内核自报成败、能在主机上跑单元测试。下一卷我们正式进实模式——正文 [001 · 实模式引导](../../book/01-boot/001/) 里,你会亲手敲下 `make run`,看着 BIOS 把这章拼好的镜像第 0 扇区读进 `0x7C00`,点亮黑屏。到那时,这一章的 `build_image.sh` 和 `run` 目标,就是"机器真正动起来"的那只手。
