 ...
 */
static Elf64_Ehdr* build_elf_header(uint64_t entry = 0x1000,
                                    uint64_t phoff = sizeof(Elf64_Ehdr),
                                    uint16_t phnum = 1,
                                    uint16_t phentsize = sizeof(Elf64_Phdr)) {
    reset_buffers();
...
static void add_phdr(uint16_t index, uint64_t offset, uint64_t vaddr, uint64_t paddr,
    ...
    auto* phdr = reinterpret_cast<Elf64_Phdr*>(
        g_elf_buffer + ehdr->e_phoff + static_cast<uint64_t>(index) * ehdr->e_phentsize);
 ...
 *
 * Entry = 0xFFFFFFFF80100000 should become 0x100000
 */
TEST("elf: load_elf higher-half entry point conversion") {
    ...
    uint64_t staging_size = TEST_ELF_BUFFER_SIZE;
    uint64_t entry = load_elf(g_elf_buffer, staging_size);
    ASSERT_EQ(entry, 0x100000u);
}

// ============================================================
// 16. load_elf - Segment Copy and BSS Zeroing
// ============================================================
...
TEST("elf: load_elf copies file data and zeroes BSS") {
    ...
    add_phdr(0, seg_offset, /*vaddr=*/0x1000, /*paddr=*/dest_addr,
             filesz, memsz);
...
TEST("elf: load_elf segment with no BSS") {
    ...
    add_phdr(0, seg_offset, /*vaddr=*/0x2000, /*paddr=*/dest_addr,
             seg_size, seg_size);
...
TEST("elf: load_elf two PT_LOAD segments") {
    ...
    add_phdr(0, seg1_offset, /*vaddr=*/0xFFFFFFFF80100000ULL, /*paddr=*/dest1,
             seg1_filesz, seg1_memsz);
    add_phdr(1, seg2_offset, /*vaddr=*/0xFFFFFFFF80102000ULL, /*paddr=*/dest2,
             seg2_filesz, seg2_memsz);

    uint64_t staging_size = TEST_ELF_BUFFER_SIZE;
    uint64_t entry = load_elf(g_elf_buffer, staging_size);

    // Entry: 0xFFFFFFFF80100000 - 0xFFFFFFFF80000000 = 0x100000
    ASSERT_EQ(entry, 0x100000u);
