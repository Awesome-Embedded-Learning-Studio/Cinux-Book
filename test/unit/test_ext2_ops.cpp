...
static void init_sim_disk(SimDisk& disk, uint32_t groups = 1) {
    ...
    uint32_t inode_table_blocks = (TEST_IPG * TEST_INODE_SIZE + TEST_BLOCK_SIZE - 1)
        / TEST_BLOCK_SIZE;
    ...
    for (uint32_t g = 0; g < groups; ++g) {
        ...
        disk.bgdt[g].bg_free_blocks_count = static_cast<uint16_t>(
            TEST_BPG - 2 - inode_table_blocks);
    ...
    uint32_t root_data_blk_offset = 2 + (TEST_IPG * TEST_INODE_SIZE + TEST_BLOCK_SIZE - 1)
        / TEST_BLOCK_SIZE;
    ...
    for (uint32_t g = 0; g < disk.group_count; ++g) {
        ...
        for (uint32_t byte_idx = 0; byte_idx < bytes_needed; ++byte_idx) {
            ...
            for (uint32_t bit = 0; bit < 8; ++bit) {
                ...
                if (local >= TEST_BPG) break;
                if ((disk.block_bitmaps[g][byte_idx] & (1U << bit)) == 0) {
                    disk.block_bitmaps[g][byte_idx] |= static_cast<uint8_t>(1U << bit);
                    uint32_t global = g * TEST_BPG + TEST_FDB + local;
                    if (disk.sb.s_free_blocks_count > 0) --disk.sb.s_free_blocks_count;
                    if (disk.bgdt[g].bg_free_blocks_count > 0) --disk.bgdt[g].bg_free_blocks_count;
                    return global;
                }
            }
        }
...
static bool host_free_block(SimDisk& disk, uint32_t blk) {
    ...
    if (g >= disk.group_count) return false;
...
static uint32_t host_alloc_inode(SimDisk& disk) {
    for (uint32_t g = 0; g < disk.group_count; ++g) {
        if (disk.bgdt[g].bg_free_inodes_count == 0) continue;
        uint32_t bytes_needed = TEST_IPG / 8;
        for (uint32_t byte_idx = 0; byte_idx < bytes_needed; ++byte_idx) {
            if (disk.inode_bitmaps[g][byte_idx] == 0xFF) continue;
            for (uint32_t bit = 0; bit < 8; ++bit) {
                uint32_t local = byte_idx * 8 + bit;
                if (local >= TEST_IPG) break;
                if ((disk.inode_bitmaps[g][byte_idx] & (1U << bit)) == 0) {
                    ...
                    if (disk.sb.s_free_inodes_count > 0) --disk.sb.s_free_inodes_count;
                    if (disk.bgdt[g].bg_free_inodes_count > 0) --disk.bgdt[g].bg_free_inodes_count;
                    return global_ino;
                }
            }
        }
 ...
 */
static int64_t host_file_write(SimDisk& disk, Ext2Inode& inode,
    ...
    while (total_written < length) {
        ...
        for (uint32_t i = 0; i < chunk; ++i) {
            disk.data_blocks[disk_block][block_offset + i] =
                data[total_written + i];
        }
 ...
 */
static int64_t host_file_read(SimDisk& disk, const Ext2Inode& inode,
 ...
 */
static bool host_add_dir_entry(SimDisk& disk, uint32_t /*dir_ino*/,
                                Ext2Inode& dir_disk,
 ...
 */
static bool host_remove_dir_entry(SimDisk& disk, const Ext2Inode& dir_disk,
 ...
 */
static uint32_t host_create(SimDisk& disk, uint32_t parent_ino,
                             const char* name, uint32_t name_len) {
    ...
    if (!host_add_dir_entry(disk, parent_ino, parent, new_ino,
 ...
 */
static uint32_t host_mkdir(SimDisk& disk, uint32_t parent_ino,
                            const char* name, uint32_t name_len) {
    if (name == nullptr || name_len == 0 || name_len > EXT2_NAME_MAX) {
        return 0;
    }
    ...
    // ".." entry
    auto* dotdot = reinterpret_cast<Ext2DirEntry*>(
        disk.data_blocks[data_blk] + dot_rec_len);
 ...
 */
static int host_unlink(SimDisk& disk, uint32_t parent_ino,
                        const char* name, uint32_t name_len) {
...
static bool dir_has_entry(SimDisk& disk, const Ext2Inode& dir,
                           const char* name, uint32_t name_len,
                           uint32_t& out_ino) {
    uint32_t bs = TEST_BLOCK_SIZE;
...
TEST("file_write: write crosses block boundary") {
    ...
    uint8_t data[10];
    for (int i = 0; i < 10; ++i) data[i] = static_cast<uint8_t>('A' + i);

    int64_t written = host_file_write(disk, file_inode,
                                       TEST_BLOCK_SIZE - 5, data, 10);
    ASSERT_EQ(written, 10);
    ...
    for (int i = 0; i < 5; ++i) {
        ASSERT_EQ(disk.data_blocks[blk0][TEST_BLOCK_SIZE - 5 + i],
    ...
    for (int i = 0; i < 5; ++i) {
        ASSERT_EQ(disk.data_blocks[blk1][i],
                  static_cast<uint8_t>('F' + i));
    }
}
...
TEST("create: creates file with correct inode and directory entry") {
    ...
    ASSERT_EQ(file_inode.i_mode & EXT2_S_IFMT,
              static_cast<uint16_t>(EXT2_S_IFREG));
...
TEST("create: long filename (up to 255 chars)") {
    ...
    char name[255];
    for (int i = 0; i < 254; ++i) name[i] = 'a' + (i % 26);
    name[254] = '\0';
...
TEST("mkdir: creates directory with dot and dotdot entries") {
    ...
    ASSERT_EQ(dir_inode.i_mode & EXT2_S_IFMT,
              static_cast<uint16_t>(EXT2_S_IFDIR));
...
TEST("remove_dir_entry: clear first entry in block") {
    ...
    auto* data = disk.data_blocks[blk];
    for (uint32_t i = 0; i < TEST_BLOCK_SIZE; ++i) data[i] = 0;
...
TEST("file_write: overwrite same region updates data") {
    ...
    // First write
    uint8_t data1[50];
    for (int i = 0; i < 50; ++i) data1[i] = 'A';
    host_file_write(disk, file_inode, 0, data1, 50);

    // Overwrite
    uint8_t data2[20];
    for (int i = 0; i < 20; ++i) data2[i] = 'B';
...
TEST("file_write: i_blocks field tracks 512-byte sectors correctly") {
    ...
    // Write one block
    uint8_t data[TEST_BLOCK_SIZE];
    for (uint32_t i = 0; i < TEST_BLOCK_SIZE; ++i) data[i] = 0xAA;
