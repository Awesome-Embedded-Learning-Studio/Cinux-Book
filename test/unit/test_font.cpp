...
static FontInfo parse_psf2(const uint8_t* data, size_t len) {
    FontInfo info;
    if (len < sizeof(PSF2Header)) return info;
    const auto* hdr = reinterpret_cast<const PSF2Header*>(data);
    if (hdr->magic != PSF2_MAGIC) return info;
...
TEST("font: parse valid PSF2 header") {
    ...
    ASSERT_EQ(reinterpret_cast<uintptr_t>(info.glyphs),
              reinterpret_cast<uintptr_t>(buf) + 32);
}
