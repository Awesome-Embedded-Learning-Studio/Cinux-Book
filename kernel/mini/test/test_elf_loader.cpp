...
static void build_valid_elf(
    uint64_t entry_addr = 0x1000000,
    uint64_t paddr      = 0x1000000,
    uint64_t filesz     = 0x1000,
    uint64_t memsz      = 0x2000)
{
...
namespace test_elf_load_basic {
    ...
    void test_load_segment_copy_and_bss() {
        ...
        memset(reinterpret_cast<void*>(load_addr + filesz), 0xFF,
               static_cast<size_t>(memsz - filesz));
